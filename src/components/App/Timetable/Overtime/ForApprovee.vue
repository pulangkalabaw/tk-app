                        ...
                        <div class="panel panel-primary">
                            <div class="panel-heading">
                                <h3 class="panel-title">Overtime
                                    ...
                                    <span v-else>
                                        <span class='fa fa-refresh' data-toggle="tooltip" title="Refresh data" @click="refreshGetOvertime()"></span>
                                    </span>
                                </h3>
                            </div>
...
<script>
    ...
    computed:{
        pendingOvertime() {
            if(this.$store.state.all_overtime_requests){
                let appr = this.$store.state.all_overtime_requests
    ...
    methods:{
        ...
        refreshGetOvertime () {
            this.$store.state.all_overtime_requests = []
            this.getOvertime();
        },

        getOvertime(){

            if (this.$store.state.all_overtime_requests.length == 0) {
                this.index_request_loading = true
                ...
                .then(res => {
                    if(res.data.status == 'success'){
                        this.$store.state.all_overtime_requests = res.data.msg;
                    }
            ...
            if(conf){
                ...
                .then(res => {
                    if(res.data.status == 'success'){
                        ...
                        this.notif = '';
                        this.deleteData(this.modalform.id)
                        this.isEditing = 0;
                        $("#approvalModal").modal("toggle");
                    }else{
                        this.notif = {
        ...
        deleteData(id){
            var indexx = this.$store.state.all_overtime_requests.findIndex(x => x.id == id);
            this.$store.state.all_overtime_requests.splice(indexx, 1);
        },
